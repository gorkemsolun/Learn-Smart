from fastapi import FastAPI
from fastapi.middleware.cors import CORSMiddleware

from skill_tree_service.app.api.public import router as public_router
#from skill_tree_service.app.api.private import router as private_router
from skill_tree_service.app.util import init

# create the FastAPI app
app = FastAPI()

# re/create the database tables
init(restart=False)

# Add CORS middleware to allow cross-origin requests
# TODO: Disable this in production and specify the frontend URL
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_credentials=True,
    allow_methods=["*"],
    allow_headers=["*"],
)
app.include_router(public_router, prefix="/api")
#app.include_router(private_router, prefix="/api")

print("FastAPI Skill Tree service started successfully")
