...
import {NodeData, LinkData, CustomSimulationNode, SkillTree} from "@/app/types";
...
export default function SkillTree() {
  // TO-D0 write the backend and take skill tree using router of skill tree
  const params = useParams<{ id: string }>();
  const skill_tree_id = params.id;
  ...
  useEffect(() => {
    if (!svgRef.current || !skillTree.nodes || !skillTree.nodeLinks) return;
    ...
    const hierarchy = d3.stratify<NodeData>()
      .id((d) => d.id)
      .parentId((d) => skillTree.nodeLinks.find((link) => link.target === d.id)?.source)(skillTree.nodes)
    ...
    const simulation = d3
      ...
      .force(
        ...
        d3.forceLink<CustomSimulationNode>()
          .links(skillTree.nodeLinks.map(link => ({
